...
import {KitDetail} from "@modules/logistics/catalogue/kit/models/kit";
...
@Component({
    ...
    template: `
        <sb-layout-dashboard>
            <sb-dashboard-head title="Nuevo Kit" [hideBreadcrumbs]="false"></sb-dashboard-head>
            <sb-card>
                <div class="card-body">
                    ...
                    <div class="form-group col-md-12 text-right">
                        ...
                        <button class="btn btn-success btn-sm ml-3" [disabled]="kitForm.invalid"
                                (click)="onSave()">
...
export class KitNewComponent implements OnInit {
    ...
    public onSave(): void {
        if (this.kitForm.valid && this.kitDetails.length > 0) {
            ...
            this.confirmDialogService.confirmSave().then(() => {
                this.kitService.add$(this.kitForm.value).subscribe(response => {
                    if (response) {
                        this.onBack();
                    }
                }, error => {
